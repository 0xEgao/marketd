    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-900">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-orange-400 text-xl font-medium">Loading Market Data...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="max-w-screen-2xl mx-auto p-6">
        <div className="mb-8">
          <h1 className="text-5xl font-bold text-orange-500 mb-2">Coinswap Market</h1>
          <p className="text-gray-400 text-lg">Live market offers with competitive rates</p>
        </div>
              ...
              <tbody className="divide-y divide-gray-700">
                {offers.map((offer, index) => (
                  <tr key={index} className="hover:bg-gray-700 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4">
                      <span className="font-medium text-green-400">{offer.base_fee}</span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-yellow-400">{offer.min_size.toLocaleString()}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-yellow-400">{offer.max_size.toLocaleString()}</span>
                    </td>
                    <td className="px-6 py-4">
        ...
        <div className="mt-6 text-center text-gray-400 text-sm">
          <p>Showing {offers.length} active offers • Last updated: {new Date().toLocaleTimeString()}</p>
        </div>
      </div>
    </div>
  );
